...
const ProfileCard = ({ socialData }) => {
  const { theme, toggleTheme } = useContext(ThemeContext);
  const experience = calculateExperience(2021) - 1; // Since 2021

  const email = socialData?.contact?.email || "me@nextsaimon.com";
  const storageApi = process.env.NEXT_PUBLIC_STORAGE_API;

  const getSocialUrl = (platform, fallback) => {
    const found = socialData?.socials?.find((s) => s.platform.toLowerCase() === platform.toLowerCase());
    return found ? found.url : fallback;
  };

  const facebook = getSocialUrl("facebook", "https://www.facebook.com/nextsaimon");
  const github = getSocialUrl("github", "https://github.com/nextsaimon");
  const instagram = getSocialUrl("instagram", "https://www.instagram.com/nextsaimon");
      ...
      >
        <section className="bg-[var(--card-bg-color)] border border-[var(--border-color)] rounded-2xl p-8 flex flex-col items-center text-center shadow-lg">
          ...
          <div className="relative mb-6 group">
            <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-[var(--border-color)] shadow-md transition-transform duration-500 group-hover:scale-105">
              <Image
                ...
                src={`${storageApi}/profile_pic/profile-pic.png`}
                width={128}
                height={128}
                quality={100}
                priority
          ...
          <p className="text-xs text-[var(--secondary-text-color)] mt-4 leading-relaxed max-w-[240px]">
            I am a CSE student with {experience}+ years of coding experience. I love learning new tech stacks.
          </p>
            ...
            >
              <div className="flex items-center gap-3">
                <BsFacebook
                  ...
                  aria-label="Facebook"
                />
                {/* <span className="w-5 h-5 rounded-full border border-[var(--border-color)] flex items-center justify-center font-bold bg-[var(--bg-color)] transition-colors group-hover:bg-[var(--border-color)]">F</span> */}
