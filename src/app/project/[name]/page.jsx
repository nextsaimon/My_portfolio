  ...
  useEffect(() => {
    ...
    if (name) {
      ...
      fetch(`${storageApi}/jsons/project_data.json`)
        ...
        .then((data) => {
          const foundProject = data.find((p) => p.name === name);
          if (foundProject) {
            setProject(foundProject);

            if (foundProject.readme) {
              const cleanReadmePath = foundProject.readme.startsWith("/")
                ? foundProject.readme
                : `/${foundProject.readme}`;
              const readmeUrl = foundProject.readme.startsWith("http")
                ? foundProject.readme
                : `${storageApi}${cleanReadmePath}`;
              fetch(readmeUrl)
                .then((res) => res.text())
                .then((text) => {
